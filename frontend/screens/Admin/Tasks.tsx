...
const Tasks: React.FC = () => {
  ...
  const fetchTasks = async () => {
    ...
    if (searchText) activeFilters.search = searchText;
    if (assigneeEmail) activeFilters.assigneeEmail = assigneeEmail;
    if (ownerEmail) activeFilters.ownerEmail = ownerEmail;
    try {
      const response = await getAdminTasks(activeFilters);
  ...
  const closeEditTaskModal = () => {
    setEditModalVisible(flase);
    fetchTasks();
  }

  useEffect(() => {
    fetchTasks();
  }, [sortBy, priorityFilter, statusFilter, dayFilter, searchText, assigneeEmail, ownerEmail]);

  const handleSearch = async (search: string) => {
    setSearchText(search);
    await fetchTasks({ searchText, assigneeEmail, ownerEmail });
  };

  const handleSearchByAssignee = async (email: string) => {
    setAssigneeEmail(email);
    await fetchTasks({ search: searchText, assigneeEmail: email, ownerEmail });
  }
  const handleSearchByOwner = async (email: string) => {
    setOwnerEmail(email);
    await fetchTasks({ search: searchText, assigneeEmail, ownerEmail: email });
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        value={assigneeEmail}
        onChangeText={handleSearchByAssignee}
      />
      <TextInput
        style={styles.searchInput}
        placeholder="Enter owner email"
        value={ownerEmail}
        onChangeText={handleSearchByOwner}
      />
      {/* Filter Section */}
      <View style={styles.filterContainer}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          ...
          <TouchableOpacity style={[styles.filterButton, dayFilter && styles.activeFilterButton]} onPress={() => setDayModalVisible(true)}>
            <Text style={styles.filterButtonText}>Created At: {dayFilter || "All"}</Text>
          </TouchableOpacity>
      ...
      <View>
        {isLoading ? (
          ...
          tasks.length === 0 ? (
            // Display this message if no tasks are found
